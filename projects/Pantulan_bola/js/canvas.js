...
function drawTrajectory(ctx, simulationData, frameIndex, ballIndex, isDark) {
  ...
  if (frameIndex < simulationData.length) {
    ...
    drawBall(ctx, ball, ballX, ballY, ballIndex);
...
/**
 * Menggambar bola dengan gambar PNG
 * KUNCI: Menggunakan scale dari data.js
 */
function drawBall(ctx, ball, x, y, ballIndex) {
  const ukuranDasar = 40; // Ukuran standar di canvas
  const ukuran = ukuranDasar * ball.scale; // Scale sesuai jenis bola
  ...
  if (img && img.complete) {
    ...
    // Fallback jika gambar belum load
